...
from ᴋʟᴀx.log_channel import gloggable


BLACKLISTWHITELIST = [OWNER_ID] + DEV_USERS + KLAW_LINGS
...
def unbl_user(update: Update, context: CallbackContext) -> str:
    ...
    if sql.is_user_blacklisted(user_id):
        ...
        message.reply_text(f"{ALKL}I am not ignoring them at all though!")
        return ""
...
def bl_users(update: Update, context: CallbackContext):
    ...
    for each_user in sql.BLACKLIST_USERS:
        ...
        if reason:
            users.append(
                f"{ALKL}• {mention_html(user.id, html.escape(user.first_name))} :- {reason}"
            )
        else:
            users.append(f"{ALKL}• {mention_html(user.id, html.escape(user.first_name))}")

    message = f"{ALKL}<b>Blacklisted Users</b>\n"
    if not users:
...
__element__ = "Blacklisting Users"
